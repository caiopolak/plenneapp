...
serve(async (req) => {
  ...
  try {
    ...
    if (!plan || !PLAN_PRICES[plan as keyof typeof PLAN_PRICES]) {
      throw new Error("Invalid plan. Use 'pro' or 'business'");
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/app/subscription?success=true`,
      cancel_url: `${req.headers.get("origin")}/app/subscription?canceled=true`,
      metadata: {
        user_id: user.id,
        plan: plan
      }
